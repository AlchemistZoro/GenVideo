...
async def write_voice_info(task):
    for hash_code, infos in voice_infos.items():
        voice_info_path = os.path.join('log', task, 'voice_info', f'{hash_code}.json')
        os.makedirs(os.path.dirname(voice_info_path), exist_ok=True)
        with open(voice_info_path, 'w', encoding='utf-8') as f:
            json.dump(infos, f, ensure_ascii=False, indent=4)

async def main(task, voice_config):
    ...
    for hash_code in os.listdir(text_path):
        if hash_code.endswith('.json') and not os.path.exists(os.path.join(voice_dir,hash_code[:-5])):
            ...
            for text_index in range(len(texts)):
                ...
                video_type = texts[text_index]['video_type']
                tasks.append(process_text_voice(text, config, hash_code[:-5], text_index, voice_dir, task, video_type))

            await asyncio.gather(*tasks)
        time.sleep(1)
        # break  # 如果您不希望在处理一个文件后就停止，请删除此行

    # 所有任务完成后，写入voice_info
    await write_voice_info(task)
...
if __name__ == "__main__":
    ...
    asyncio.run(main(task, voice_config))
